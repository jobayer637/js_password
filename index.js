...
class myClass {
    ...
    myPassword() {
        let str = this.str || this.randomPassword()
        const sc = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/
        const s = /[a-zA-Z]/
        const n = /[0-9]/
...
$("#text").on('click keyup keypress change', function () {
    let myObj = new myClass($(this).val())
    if ($(this).val().length > 5) {
        $("#show").text(myObj.myPassword())
    } else {
...
$("#randomPass").on('click', function () {
    let randObj = new myClass
    $("#text").val(randObj.randomPassword())
    $("#show").text(randObj.myPassword())
})